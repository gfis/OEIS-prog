...
sub gener { # writes global $next_code, $methods[$imethod]
    ...
    $methods[$imethod] = "~~  ~~~~private Z t0 = Z.valueOf(++mN)";
    for (my $ireg = 1; $ireg <= $max_reg; $ireg ++) {
        $methods[$imethod] .= ", t$ireg = Z.ZERO";
    } # for ireg
    $methods[$imethod] .= ";";
    my $code .=  "~~    " .&expand_loop(0, scalar(@lines), 0, 0, ""); # recursively expand because of lpb/lpe; loopreg=0 for outmost level
    return $code;
} # gener
#--
sub expand_loop {
    ...
    while ($iline < $to) {
        ...
        if (0) {
            ...
            my $ilpe = &search_lpe($iline);

            # build the call "loopi();"
            $imethod ++; # push
            my  $suffix = 0;
            ...
            $result .= "~~${indent}$method_name();";

            # build the head of the called method
            $methods[$imethod] .= "~~private Z $method_name() {";
            for (my $ireg = 0; $ireg <= $max_reg; $ireg ++) {
                $methods[$imethod] .= "~~  Zs$ireg = t$ireg;"; # save all
            } # for ireg
            ...
            for (my $ireg = $loopreg; $ireg < $loopreg + $range; $ireg ++) {
                $methods[$imethod] .= "~~    t$ireg = t$ireg.subtract(1);"; # decrement all in range
            } # for ireg
            $methods[$imethod] .= "~~    if"; 
            my $sep = "(";
            for (my $ireg = $loopreg; $ireg < $loopreg + $range; $ireg ++) {
                $methods[$imethod] .= $sep . "t$ireg >= 0 && t$ireg < s$ireg"; # compare the range
                $sep = " && ";
            } # for ireg
            $methods[$imethod] .= ") {~~    busy = false;"; # start "if"
            for (my $ireg = $loopreg; $ireg < $loopreg + $range; $ireg ++) {
                $methods[$imethod] .= "~~    t$ireg = s$ireg;"; # rollback
            } # for ireg
            $methods[$imethod] .= "~~    } else {";
            for (my $ireg = $loopreg; $ireg < $loopreg + $range; $ireg ++) {
                $methods[$imethod] .= "~~    s$ireg = t$ireg;"; # save again
            } # for ireg
            $methods[$imethod] .= "~~    } // if"; # end of "if", "while"
            $methods[$imethod] .= "~~  } // while"; # end of "if", "while"
            $imethod --; # pop
...
sub search_lpe {
    ...
    return $iline - 1; # index of corresponding lpe
} # search_lpe
#--
sub vali {
    my ($expr) = @_;
