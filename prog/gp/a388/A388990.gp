/* source=https://oeis.org/A388990 lang=pari curno=1 type=an rev=9 offset=1 bfimax=22155 */
;
up_to_cols = 210;
A326127(n) = (sigma(n)-core(n)-n);
A326128(n) = (n-core(n));
A388992(n) = { my(a=A326127(n)); (a / gcd(a, A326128(n))); };
...
A388990sq(row,col) = A388992(A388981sq(row,col));
A388990list(up_to) = { my(v = vector(up_to), i=0); for(a=1,oo, for(col=1,a, i++; if(i > up_to, return(v)); v[i] = A388990sq(col,(a-(col-1))))); (v); };
v388990 = A388990list(up_to);
A388990(n) = v388990[n];
a(n)=A388990(n);
