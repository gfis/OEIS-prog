/* source=https://oeis.org/A143696 lang=pari curno=1 type=an rev=12 offset=1 bfimax=32 nstart=1 */
...
a(n) = {expz = 2^valuation(n, 2); y = n/expz; d = csiz(y, 2); prod(i=1, length(d), 1 + (2^(expz*d[i])-1)*(2^d[i]+1)/(2^d[i]-1));};
a(n);
